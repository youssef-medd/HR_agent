...
def test_sender_impls_render_and_log(db, monkeypatch):
    ...
    _sent_log_reset()
    rej = _send_rejection_impl(db, 1, "cand@x.io", "rejection@v1")
    off = _send_offer_impl(db, 2, "cand2@x.io", "offer@v1")
...
def test_notification_routes_to_whatsapp_for_phone(db, monkeypatch):
    ...
    _sent_log_reset()
    entry = _send_confirmation_impl(db, 1, "21693008267")
